...
export function AzureMigrationWhyNetsmartz() {
  const reasons = [
    { 
      ...
      <div className="max-w-[1100px] w-full text-center mb-16">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
          {reasons.map((reason, idx) => (
            <div 
              key={idx} 
              className="bg-[#FFF4EB] border border-[#f06f12]/20 rounded-[4px] py-8 px-4 flex flex-col items-center justify-start text-center shadow-sm min-h-[160px] transition-shadow hover:shadow-md"
            >
              {reason.icon}
          ...
          <div className="relative z-10 flex flex-col h-full justify-center">
            <div>
              <h2 className="text-3xl md:text-[38px] font-bold leading-[1.1] tracking-tight mb-8">
                See How Netsmartz<br className="hidden md:block"/> Can Work For You
              </h2>
            ...
            <div className="mt-6">
              <p className="font-bold text-[14.5px] leading-relaxed">
                ...
                <a href="mailto:sales@netsmartz.com" className="text-[#f06f12] hover:text-[#d5610e] transition-colors underline decoration-[#f06f12]/30 underline-offset-4">sales@netsmartz.com</a>
              </p>
            </div>
          </div>
        </div>
          ...
          <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
            ...
            <div>
              <input 
                type="text" 
                placeholder="Name*" 
                className="w-full px-4 py-[13px] bg-white border border-gray-300 rounded-[4px] text-[13px] placeholder:text-gray-400 focus:outline-none focus:border-accent-gold transition-colors"
                required
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input 
                ...
                required
              />
              <div className="relative flex bg-white items-center border border-gray-300 rounded-[4px] focus-within:border-accent-gold transition-colors overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-5 pt-5">
              <button 
                type="submit" 
                className="bg-[#f06f12] hover:bg-[#d5610e] text-white font-bold py-3 px-10 rounded-[4px] text-[14px] transition-all w-full sm:w-auto shadow-sm"
              >
                Submit
              </button>
