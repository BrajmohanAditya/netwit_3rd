        ...
        >
          {partners.map((partner) => (
            <motion.div
              ...
              transition={{ duration: 0.4, ease: "easeOut" }}
              className="flex h-16 w-32 items-center justify-center rounded-lg border border-muted/20 bg-surface px-4 transition-colors hover:border-primary/30 hover:bg-primary/5"
            >
              <span className="font-syne text-lg font-bold text-muted">{partner}</span>
            </motion.div>
          ))}
        </motion.div>
      </motion.div>
