    ...
    <section className="bg-[#020617] py-24 relative overflow-hidden font-sans border-t border-white/5">
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20 space-y-6">
           <span className="text-[10px] text-amber-500 font-black uppercase tracking-[0.5em] italic">The Fiscal Arsenal</span>
           <h2 className="text-4xl md:text-6xl font-black text-white tracking-tighter italic shadow-sm">Unified <span className="text-amber-500 underline decoration-amber-500/30">Capital Stack.</span></h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {toolsData.map((category, idx) => (
            <div 
              key={idx} 
              className={`group relative bg-[#0f172a]/40 backdrop-blur-3xl border border-white/5 rounded-[3rem] p-10 overflow-hidden transition-all duration-500 hover:border-amber-500/30 hover:-translate-y-2 shadow-2xl`}
            >
              <div className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r ${category.color}`}></div>
              <div className="absolute top-0 right-0 w-32 h-32 bg-amber-500/5 blur-[50px] pointer-events-none group-hover:bg-amber-500/10 transition-colors"></div>

              <div className="flex items-center gap-5 mb-8">
                 <div className="w-14 h-14 rounded-2xl bg-amber-600/10 border border-amber-600/20 flex items-center justify-center group-hover:bg-amber-600 group-hover:text-white transition-all shadow-lg">
                    <category.icon className="w-7 h-7" />
                 </div>
                 <h3 className="text-xl font-black text-white italic tracking-tight uppercase underline decoration-amber-600/20">{category.title}</h3>
              </div>
              ...
              <div className="mt-8 pt-8 border-t border-white/5 flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity">
                 <span className="text-[10px] text-amber-500 font-black uppercase tracking-widest italic">Institutional Registry</span>
