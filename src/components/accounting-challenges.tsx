    ...
    <section className="bg-[#020617] py-24 relative overflow-hidden font-sans border-t border-white/5">
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20 space-y-6">
           <span className="text-[10px] text-amber-500 font-black uppercase tracking-[0.5em] italic">The Architectural Solution</span>
           <h2 className="text-4xl md:text-6xl font-black text-white tracking-tighter italic">Solve Your <span className="text-amber-500 underline decoration-amber-500/30">Fiscal Challenges.</span></h2>
        ...
        <div className="grid lg:grid-cols-[1fr,1.5fr] gap-12 items-center">
               ...
               {challenges.map((step) => (
                  <button
                    ...
                    className={`group relative p-8 rounded-[2.5rem] text-left transition-all duration-500 border overflow-hidden ${
                      activeStep === step.id 
                        ? "bg-amber-600 border-amber-500 shadow-[0_20px_60px_rgba(251,191,36,0.3)] scale-[1.02]" 
                        : "bg-[#0f172a]/40 border-white/5 hover:border-amber-500/20"
                    }`}
                  >
                     <div className="flex justify-between items-center relative z-10">
                        <div className="flex items-center gap-5">
                            <span className={`text-4xl font-black italic tracking-tighter ${activeStep === step.id ? "text-white/20" : "text-amber-500/10 group-hover:text-amber-500/20 transition-colors"}`}>0{step.id}</span>
            ...
            <div className="relative h-[500px] perspective-[1400px]">
               <div className="w-full h-full bg-gradient-to-br from-amber-900/10 via-[#0f172a] to-blue-900/10 border border-amber-500/20 rounded-[4rem] p-16 flex flex-col justify-center relative shadow-2xl backdrop-blur-3xl group overflow-hidden">
                  
                  {/* Abstract Background Element */}
                  <div className="absolute -top-20 -right-20 w-64 h-64 bg-amber-500/10 rounded-full blur-[100px] animate-pulse"></div>

                  <div className="relative z-10 space-y-10 animate-in fade-in slide-in-from-right-8 duration-500" key={activeStep}>
                     <div className="flex items-center gap-4">
                        <span className="text-[10px] text-amber-400 font-black uppercase tracking-[0.5em] italic">Solution Registry</span>
                     ...
                     <h2 className="text-4xl md:text-5xl font-black text-white italic tracking-tighter leading-tight uppercase underline decoration-amber-600/30">
                        {challenges.find(s => s.id === activeStep)?.title}
                     </h2>

                     <p className="text-xl md:text-2xl text-slate-400 font-light leading-relaxed border-l-2 border-amber-500/30 pl-10 italic">
                        {challenges.find(s => s.id === activeStep)?.content}
                     </p>

                     <div className="pt-10 flex items-center gap-6">
                        <div className="w-12 h-12 rounded-xl bg-amber-600/10 border border-amber-600/20 flex items-center justify-center">
